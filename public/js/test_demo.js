...
function getlist(lat, lng, isVacant) {
    fetch(buildUrl('/getTotalPages', {lat: lat, lng: lng, zip: postalcode, isVacant: isVacant}), {
        ...
        .then(function (response) {
                ...
                let location = [];
                searchCount = 0;
               /* var propertyAll = postData('/allpropertiesList', {
                        lat: lat,
                        lng: lng,
                        page: 1,
                        zip: postalcode
                    }, isVacant);*/
                var propertyAll = JSON.parse(localStorage.getItem("property"));
                console.log(propertyAll['property'].length);
                $("#poiContent").hide();
                for (let i = 1; i <= totalPages; i++) {
                    if (propertyAll) {
                        bar1.set((i / parseInt(totalPages)) * 100);
                        if (i == totalPages) {
                            $("#searchloading").fadeOut("slow", function () {
                ...
                if (document.URL.includes('hpl2')) {
                    ...
                    if (totalPages == propertyAll.status.page) {
...
function f(locations) {
    ...
    /*for (let i = 0; i < locations.length; i++) {
        // console.log("Value "+i+": "+locations[i][0]+", "+locations[i][1])
        ...
        if (status == google.maps.GeocoderStatus.OK) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    // polys = [data];
                    // localStorage.setItem("polys", [data]);
                    // polys = fromWKT2Json(data);
                    // console.log(polys);
                    init();
                },
                cache: false,
...
function init() {
    ...
    ///////////////////////////////////////   
        for (var i = 0; i < polygon.getPath().getLength(); i++) {    
            ...
            polygonArray= polygon.getPath().getAt(i) ;
            $('.swiper-wrapper').empty();
            ...
            var isWithinPolygon = google.maps.geometry.poly.containsLocation(coordinate, polygon);
                ...
                if(isWithinPolygon){
                    count = j;
                    // count ++;
                    // console.log(locationLatLng[j][0], locationLatLng[j][1]);
                    $('#searchCount').text("Property count : " + latlongCount.length);
                    $("#poiContent").show();         
                    markers = new google.maps.Marker({
                        position: coordinate,
                        animation: google.maps.Animation.DROP,
                    ...
                    var text = '<div class="swiper-slide" style="height: 100px;">' +
                        '<div class="box selectPOI">' +
                        '<span class="h3 hotlineLabel ' + "visited" + '" target="_blank" lat ="' + locationLatLng[j][0] + '" long = "' + locationLatLng[j][1] + '" line1 = "' + encodeURI(locationLatLng[j][2]) + '" line2="' + encodeURI(locationLatLng[j][2]) + '" > Hot Property </span>' +
                        '<div class="float-right">' +
                        '<input type="checkbox" name="selectedItem" class="selectedProperty" aria-label="Checkbox for following text input">' +
                        ...
                        '</div>' +
                        '<div class="float-left">' +
                        '<img width="250px" src="https://maps.googleapis.com/maps/api/streetview?size=250x250&location=' + locationLatLng[j][0] + ',' + locationLatLng[j][1] + '&pitch=-0.76&key=AIzaSyChy0iFCguYHXfzxP_G1L1knHzvImm8VcQ" alt="">' +
                        '</div></div></div>';
                    swiper.appendSlide(text);
