...
class AjaxController extends Controller
{
    ...
    // private $obapiurl = 'https://api.gateway.attomdata.com', $obapikey = 'b93b589b4e663e106cc6ef299be0c2c2';
    private $obapiurl = 'https://api.gateway.attomdata.com', $obapikey = 'e702e26e93b183f8d1dfd7ba9dc05390';
    ...
    public function getzipResponse(Request $request)
    {
        ...
        return $response;*/
        // $AreaHierarchy = $this->getAreaHierarchy(45.553186, -122.681354);
        ...
        // dd($geoARRAY);   
        foreach ($geoARRAY as $key => $value) {
            if($key === 1){
                $url = $this->obapiurl . '/areaapi/v2.0.0/boundary/detail?AreaId=' . $value . '&debug=True';
                $boundary[] = $this->curlPOIAPI($url);
            }
        }
        // dd($boundary);
        // $boundary = $this->getAreaBoundary($geoARRAY[0]);
        if($boundary[0] != null)
            return response($boundary[0]['response']['result']['package']['item'][0]['boundary']);
        else
            return response($boundary[0]['response']['result']['package']['item'][0]['boundary']);
    }
    ...
    private function getAreaHierarchy($lat, $long)
    {
        ...
        $url = $this->obapiurl . "/areaapi/v2.0.0/hierarchy/lookup?latitude=".$lat."&longitude=".$long;
        // $url = $this->obapiurl . "/areaapi/v2.0.0/hierarchy/lookup?WKTString=POINT(" . $location . ")&geoType=ZI&debug=True";
        return $this->curlPOIAPI($url);
    }

    //New function added by Bhavana 
    private function getAreaHierarchyNew($lat, $long)
    {
        $location = urlencode($long . ',' . $lat);
        // $url = $this->obapiurl . "/areaapi/v2.0.0/hierarchy/lookup?latitude=".$lat."&longitude=".$long."";
        $url = $this->obapiurl . "/areaapi/v2.0.0/hierarchy/lookup?WKTString=POINT(" . $location . ")&geoType=ZI&debug=True";
    ...
    public function ExtendedDetail(Request $request, $line1, $line2)
    {
        ...
        if ($request->user()->authorizeRoles(['user'])) {
            ...
            foreach ($AVMResult["property"] as $key => $data) {
                ...
                // var_dump($AssessmentHistory["property"]);
    ...
    public function getHouseInventry(Request $request)
    {
        ...
        return $response;*/
        // return "success";
        $AreaHierarchy = $this->getAreaHierarchyNew($lat, $lng);
    ...
    public function getTotalPages(Request $request)
    {
        ...
        // dd($getCurrentUser->Historicsavedcount);
